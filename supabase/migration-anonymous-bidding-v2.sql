-- Migration: pujas anónimas (v2 — versión robusta, dropea CUALQUIER place_bid previo)
--
-- Idempotente: se puede correr todas las veces que quieras. Borra todas las
-- variantes anteriores de place_bid (la original con auth, la versión con
-- contact info, etc.) y crea la nueva firma sin auth, con datos del pujador.

-- 1. Columnas en bids (idempotente)
...
-- 2. Drop dinámico de TODAS las versiones de place_bid en el schema public
do $$
declare
  r record;
begin
...
-- 3. Crear la nueva versión, sin auth.
